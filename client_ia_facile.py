			...
			if message=="Debut de la partie" :
				...
				if msg == "Bateaux joueur 2 placés":
					...
					while partie != "END" :
						...
						print("Le plateau d'attaque est :\n", plateau_attaque)
						print("CELL")
    	...
    	if humain: 
        	...
        	if message=="Hello i want to play" : 
        		...
        		if msg == "Bateaux joueur 1 placés":
        			...
        			# Debut de la partie #
        			while partie != "END":
        				#print("Votre plateau est :", plateau_IA)  # On peut decomenter pour bosver le plateau de l'IA
        			...
        			#On rappelle les plateaux de jeu à chaque tour#
        				...
        				message = joueur.recv()
        				msg = msg.encode()
        				#En mode facile, l'IA joue aléatoirement#
        				(a, b) = tir_random(plateau_attaque)
        				message = convertir_tir_number_to_letter(a, b)
        				msg = message.encode()
        				joueur.send(msg)
        				...
        				resultat = res.decode()
        				if resultat == "1":
        					plateau_attaque[a][b-1] = 1
        				elif resultat == "0":
        					plateau_attaque[a][b-1] = -1
        				elif resultat == "2":
        					plateau_attaque[a][b-1] = 2
        				if test_fin(plateau_attaque):
        					partie = "END"
        					var = False
        					message = "WINNER:IA"
        					msg = message.encode()
        					joueur.send()
        				#L'IA se place en attente de son tour#
